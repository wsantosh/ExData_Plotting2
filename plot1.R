...
#total emissions group by individual years(1999, 2002, 2005, and 2008)
totalEmissionsData<-with(NEI, aggregate(NEI$Emissions, by=list(NEI$year), sum))

#give proper heading for all columns
names(totalEmissionsData) <- c("Year", "Total PM2.5 emission")

##Construct the plot and save itEmission(Tons)G file with a width of 480 pixels and 
...
plot(totalEmissionsData, col = "red", type="l", main="Total PM2.5 Emissions in the United States(Tons)", lwd=2)
